...
def train_model(
    ...
    if save_checkpoint and dir_checkpoint is None:
        raise ValueError('dir_checkpoint must be specified if save_checkpoint is True')
    
    if train_dataloader is None:
        raise ValueError('train_dataloader must be specified')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_checkpoint:
            ...
            # @todo: commenting out below may need to be fixed
...
def train_unet(model, epochs, batch_size, lr, load, scale, amp, train_dataloader, val_dataloader, dir_checkpoint):
    #args = get_args()
